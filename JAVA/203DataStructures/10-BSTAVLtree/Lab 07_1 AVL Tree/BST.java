...
//use helper method
	...
	public void breadthFirstTraversal(TreeNode<E> root){
		...
		if(root == null) return;
		queue.add(root.element);
		while (!queue.isEmpty()){
			TreeNode<E> levelNode = (TreeNode<E>) queue.poll();
			if(levelNode.left !=null)
				queue.add((E) levelNode.left);
			if(levelNode.right !=null)
				queue.add((E) levelNode.right);
	...
	private int getNumberOfLeaves(TreeNode<E> root2) {
		if(root2 == null) return 0;
		if(root2.left == null && root2.right == null){
			return 1;
		} else {
			return getNumberOfLeaves(root2.left) + getNumberOfLeaves(root2.right);
		}
	}
